...
class OpenAIProvider(
    ...
    override val agentType: AIAgentType = AIAgentType.OPENAI
    ...
    override suspend fun generateContent(
        ...
        return try {
            ...
            "Generated content from OpenAI (placeholder)"
        } catch (e: Exception) {
            throw AIProviderException("Failed to generate content with OpenAI: ${e.message}", e)
        }
    }
    ...
    override suspend fun analyzeSources(sources: List<Source>, extractionPrompt: String): String {
        ...
        return try {
            // TODO: Implement actual OpenAI API call for source analysis

            "Analyzed sources with OpenAI (placeholder)"
        } catch (e: Exception) {
            throw AIProviderException("Failed to analyze sources with OpenAI: ${e.message}", e)
        }
    }
    ...
    private fun getConfig(): AIAgentConfig {
        return configRepository.findByAgentType(agentType)
            ?: throw AIProviderException("OpenAI configuration not found")
    }
}
